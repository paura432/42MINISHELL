...
static int	malloc_len(char const *s1)
{
	...
	while (s1 && s1[i])
	{
		//control de comillado
		squote = (squote + (!dquote && s1[i] == '\'')) % 2;
		dquote = (dquote + (!squote && s1[i] == '\"')) % 2;
		if ((s1[i] == '\"' && !squote) || (s1[i] == '\'' && !dquote))
			count++;//contador de comillas simples y dobles
		i++;
	}
	//comprobar del correcto parseo
	if (squote || dquote)
		return (-1);
	return (count);
}
...
char	*ft_strtrim_all(char const *s1, int squote, int dquote)
{
	...
	i[1] = -1;
	i[0] = 0;
	//ver cuantas comillas tiene el prompt
	count = malloc_len(s1);
	if (!s1 || count == -1)
		return (NULL);
	...
	while (s1[i[0]])
	{
		...
		//comprobacion de entre comillado y solo copiar el comando

...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/10/14 23:53:25 by pau               #+#    #+#             */
/*   Updated: 2024/10/14 23:53:28 by pau              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	else if (!is_builtin(n) && n && n->full_cmd && !dir)
	{
		...
		*s = ft_split(path, ':');
		...
		if (!n->full_path || !n->full_cmd[0] || !n->full_cmd[0][0])
			mini_perror(NULL, NCMD, *n->full_cmd, 127);
	}
	return (dir);
}
...
void	get_cmd(t_prompt *prompt, t_list *cmd, char **s, char *path)
{
	...
	if (!is_builtin(n) && n && n->full_cmd && dir)
		...
		mini_perror(NULL, NDIR, n->full_path, 127);
	else if (!is_builtin(n) && n && n->full_path && \
		access(n->full_path, X_OK) == -1)
		mini_perror(NULL, NPERM, n->full_path, 126);
	if (dir)
		closedir(dir);
	ft_free_matrix(&s);
}
...
void	*exec_cmd(t_prompt *prompt, t_list *cmd)
{
	...
	if (pipe(fd) == -1)
		return (mini_perror(NULL, PIPERR, NULL, 1));
